 ...
 */
 (function($) {
	 ...
	 var methods = {
		...
		*/
		_onSubmitEvent: function(event) { ///////////  By Christopher Sebastian: Added 'event' arg.
			var form = $(this);
			var options = form.data('jqv');
			...
			/////////////////////////////////////////////////////////
			//   By Christopher Sebastian: Do not process further  //
			//   events if validation fails.  Allows me to combine //
			//   this validation with ajaxify easily:              //
			if(!r) {                                               //
			    if(event) {                                        //
		...
		*/
		_validateFields: function(form, skipAjaxValidation) {
			...
			errorFound |= !methods._checkAjaxStatus(options);   //  Uncommented by Christopher Sebastian.
		 ...
		 _ajax: function(field, rules, i, options) {
			 ...
			 if (extraDataDynamic) {
				 ...
				 for (var i = 0; i < domIds.length; i++) {
					 var id = domIds[i];
					 if ($(id).length) {
						 var inputValue = encodeURIComponent(field.closest("form").find(id).val());
			 ...
			 if (!options.isError) {
				 $.ajax({
					 ...
					 cache: false,
					 dataType: "json",
					 data: "fieldId=" + field.attr("id") + "&fieldValue=" + encodeURIComponent(field.val()) + "&extraData=" + extraData + "&" + extraDataDynamic,
					 field: field,
					 rule: rule,
					 methods: methods,
					 options: options,
