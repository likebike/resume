/**
   ...
   Updated by Christopher Sebastian to work with jQuery 1.7.  2012-04-12.

       * Switched '.attr()' to '.prop()'

           * Note, 'id' must be retrieved by '.attr()'.

   Originally From: http://code.google.com/p/ajaxify/source/browse/#svn%2Ftrunk
...
(function($) {
  ...
  $.fn.ajaxify = function(options) {
    ...
    for (var i = 0; i < this.length; i++) {
      ...
      if ($(this[i]).prop('tagName').toLowerCase() == 'a') {
      ...
      // if it's a form
      } else if ($(this[i]).prop('tagName').toLowerCase() == 'form') {
        // find the possible submission methods
        $(this[i]).find(options.buttons).each(function(i, e) {
          // and attach click handlers to each
          $(e).click(function(event) {
            $(this).before('<input type="hidden" name="' + $(this).prop('name') + '" value="' + $(this).val() + '" class="ajaxify__submitButton__"/>').prop('name', $(this).prop('name')).val($(this).val());

            // if it's an imagae, also capture the x/y co-ordinates
            if ($(this).prop('type') == 'image') {
              $(this).before('<input type="hidden" name="' + $(this).prop('name') + '_y" value="' + (event.pageY - $(this).offset().top) + '" class="ajaxify__submitButtonX__"/>');
              $(this).before('<input type="hidden" name="' + $(this).prop('name') + '_x" value="' + (event.pageX - $(this).offset().left) + '" class="ajaxify__submitButtonY__"/>');
            }
          });
        });
...
var Ajaxify = {
  ...
  process: function(e, node) {
    ...
    if ($(node).prop('tagName').toLowerCase() == 'form') {
      // set the url to the action attribute or the options url if specified on init
      options.url = (options.url) ? this.appendToURL(options.url, options.append) : this.appendToURL($(node).prop('action'), options.append);
      // set the type to the method attribute or the options type
      options.type = (options.type) ? options.type : $(node).prop('method').toUpperCase();
      // set the content type
      options.contentType = (options.contentType) ? options.contentType : $(node).prop('enctype') || 'application/x-www-form-urlencoded';
      // get the form data
      if (options.data) {
    ...
    } else if ($(node).prop('tagName').toLowerCase() == 'a') {
      // set the url to the href attribute or the options url if specified
      options.url = (options.url) ? this.appendToURL(options.url, options.append) : this.appendToURL($(node).prop('href'), options.append);
    ...
    var selector = $(node).prop('tagName').toLowerCase() + '#' + id;

    // update the element specified in options, or the parent element if not
    options.update = (options.update) ? options.update : ($(node).prop('target') ? $(node).prop('target') : $(node).parent());
